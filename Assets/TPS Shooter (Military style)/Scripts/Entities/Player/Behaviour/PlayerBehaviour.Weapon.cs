    ...
    public partial class PlayerBehaviour
    {
        ...
        public class SelectMenuWeaponUtil
        {
            ...
            public void SelectMenuWeapon()
            {
                ...
                // Find the weapon matching the saved menu tag
                if (menuWeaponTag.Length > 0)
                {
                ...
                // If no matching weapon found, keep current one from InitializeWeapon()
                if (selectedIndex == -1)
                {
                    if (menuWeaponTag.Length > 0)
                    {
                        Debug.LogWarning($"[SelectMenuWeapon] Tag '{menuWeaponTag}' not found in AllWeapons. Check that weapon GameObject tags match menu tags.");
                    }
                    selectedIndex = player.CurrentWeaponIndex;
